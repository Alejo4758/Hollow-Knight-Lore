export default function CharacterCard ({ characterData }) {
    return (
        <div className="w-full h-full max-w-sm p-5 flex flex-col justify-between relative overflow-hidden font-cinzel bg-bg-elevated/70 border border-border-main/40 rounded-2xl cursor-pointer shadow-card transition-all duration-500 group hover:border-text-primary/30 hover:shadow-accent hover:-translate-y-1">
            ...
            <div>
                <div className="flex justify-between items-center mb-4">
                    ...
                    {characterData?.type?.map (t => (
                        <span key={characterData.id} className="px-3 py-1 text-xs uppercase tracking-widest text-text-secondary bg-bg-base/80 border border-border-main rounded-full">
                            {t}
                        </span>
                    ))}
                    </div>
